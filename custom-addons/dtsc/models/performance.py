...
class Performance(models.Model):
    ...
    def action_performance_excel(self):
        ...
        for record in records:
            ...
            # ====== Sheet 2：員工請假明細 ======
            ws2 = wb.add_worksheet("員工請假明細")
            r2 = 0
            ws2.merge_range(r2, 0, r2, 12, f"{record.report_year.name}年{record.report_month.name}月 員工請假明細", title_fmt); r2 += 2
            ...
            att = self.env["ir.attachment"].create({
                "name": f"{record.report_year.name}年{record.report_month.name}月 員工請假明細.xlsx",
                "type": "binary",
                "datas": data,
