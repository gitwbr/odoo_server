...
class PriceList(models.Model):
    ...
    attribute_value_id = fields.Many2one(
        'product.attribute.value', string="屬性", required=True, ondelete='cascade')
    attr_price = fields.Float(
        related="attribute_value_id.price_extra", string="變體基礎價格")
    ...
    def write(self, values):
        ...
        if 'price_cai' in values or 'price_jian' in values:
            ...
            obj = self.env["dtsc.quotation"].search(
                [('customer_class_id', '=', self.customer_class_id.id)])
            for record in obj:
...
class Customclass(models.Model):
    ...
    selecttype = fields.Selection(
        [('a', '一般運作'), ('b', '預設用分類')], string="類型", default="a")
    nop = fields.Boolean(string="下單界面中不呈現估價", default=False)
    payfirst = fields.Boolean(string="先收款", default=False)
    sell_user = fields.Many2many("res.users", string="銷售員", domain=lambda self: [
                                 ('groups_id', 'in', self.env.ref('dtsc.group_dtsc_yw').id)])

    quotation_ids = fields.One2many(
        'dtsc.quotation', "customer_class_id", string="報價單")
    ...
    def button_after(self):
        ...
        afterrecords = self.env['dtsc.aftermakepricelist'].search(
            [('customer_class_id', '=', self.id)])
    ...
    def button_list(self):
        quotation_list = self.env["dtsc.quotation"].search(
            [('customer_class_id', "=", self.id)])
        ...
        for record in quotation_list:
            ...
            for line in record.variant_attribute_price_id:
                ...
                existing_pricelist = self.env['dtsc.pricelist'].search([
                    ('attribute_value_id', '=',
                     line.attribute_value_id.id), ('customer_class_id', '=', self.id)
                ], limit=1)
    ...
    def button_add(self):
        ...
        quotation = self.env['dtsc.quotation'].search(
            [('customer_class_id', '=', self.id)], limit=1)
...
class ProductTemplateAttributeValue(models.Model):
    ...
    def create(self, vals_list):
        ...
        for val in vals_list:
            attribute_line = self.env['product.template.attribute.line'].browse(
                val["attribute_line_id"])
            print(attribute_line.product_tmpl_id.id)
            # 先通過產品ID 查到dtsc.quotation裡面有幾個表有這個產品
            quotations = self.env['dtsc.quotation'].search(
                [("product_id", "=", attribute_line.product_tmpl_id.id), ("customer_class_id", "!=", False)])

            for quotation in quotations:
                print(quotation.id)
                qutationattr = self.env['dtsc.quotationproductattributeprice'].search(
                    [("quotation_id", "=", quotation.id), ("attribute_value_id", "=", val["product_attribute_value_id"])], limit=1)
                if not qutationattr:
    ...
    def unlink(self):
        for record in self:
            ...
            quotations = self.env['dtsc.quotation'].search(
                [("product_id", "=", record.product_tmpl_id.id), ("customer_class_id", "!=", False)])

            for quotation in quotations:
                print(quotation.id)
                qutationattr = self.env['dtsc.quotationproductattributeprice'].search(
                    [("quotation_id", "=", quotation.id), ("attribute_value_id", "=", record.product_attribute_value_id.id)], limit=1)
                if qutationattr:
                    qutationattr.unlink()
