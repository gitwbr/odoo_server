...
odoo.define('dtsc.public_web_order', function (require) {
  ...
  var CheckoutWidget = Widget.extend({
        ...
        if (selectedProductId) {
          try {
            ...
            for (let line of attributeLines) {
              ...
              var defaultOptionText = line.attribute_id[1];  // 删掉「請選擇」和「屬性」，只保留屬性名
    ...
    // 上傳失敗時在彈窗中顯示錯誤
    showUploadErrorModal: function (message) {
      var $modal = $('#uploadErrorModal');
      if (!$modal.length) {
        $modal = $('<div class="modal fade" id="uploadErrorModal" tabindex="-1" role="dialog">').append(
          $('<div class="modal-dialog" role="document">').append(
            $('<div class="modal-content">').append(
              $('<div class="modal-header">').append(
                $('<h5 class="modal-title">').text('檔案上傳錯誤'),
                $('<button type="button" class="btn-close close" data-bs-dismiss="modal" data-dismiss="modal" aria-label="Close">').append($('<span aria-hidden="true">').html('&times;'))
              ),
              $('<div class="modal-body upload-error-modal-body">'),
              $('<div class="modal-footer">').append(
                $('<button type="button" class="btn btn-secondary" data-bs-dismiss="modal" data-dismiss="modal">').text('關閉')
              )
            )
          )
        );
        $('body').append($modal);
        // 兼容不同版本的 Bootstrap，确保按钮能关闭
        $modal.on('click', '[data-dismiss="modal"], [data-bs-dismiss="modal"], .close, .btn-close', function () {
          $modal.modal('hide');
        });
      }
      $modal.find('.upload-error-modal-body').html(typeof message === 'string' ? message : (message || '上傳失敗'));
      $modal.modal('show');
    },
...
if (mismatchMessages.length > 0) {
  ...
  return;
    ...
    prepareCheckoutData: async function () {
      ...
      var uploadErrorDetails = []; // 收集所有上传错误的详细信息
      var customer_class_id_p = this.customer_class_id
      let vals = {
        project_name: $("input[name='project_name']").val(),
        customer_id: this.partner_id,
        delivery_carrier: $("select[name='delivery_char']").val(),
      ...
      $(".banner_table").each(function () {
        var $table = $(this);
        var fileName = $table.find('#project_product_name').val() || '未命名文件';
        var uploadPromise = self.uploadFile($table).then(uploadedFileName => {
          ...
          uploadErrors = true;
          // 收集错误详情
          uploadErrorDetails.push(`檔案「${fileName}」: ${error}`);
          return "";
        });
      ...
      try {
        ...
        if (!uploadErrors) {
          ...
          console.log('[DEBUG] uploadErrors is true, showing error modal');
          console.log('[DEBUG] uploadErrorDetails:', uploadErrorDetails);
          // 在弹窗中显示汇总的错误信息
          var errorSummary = '<strong>上傳失敗，請重試</strong><br><br>';
          if (uploadErrorDetails.length > 0) {
            errorSummary += '錯誤詳情：<br>' + uploadErrorDetails.join('<br>');
          } else {
            errorSummary += '部分檔案上傳失敗，請檢查檔案並重試。';
          }
          console.log('[DEBUG] calling showUploadErrorModal with:', errorSummary);
          self.showUploadErrorModal(errorSummary);
          // 处理上传错误
        }
      } catch (error) {
    ...
    createBannerTable: function (selectedCustomer, categories) {
      ...
      var headerRow = $('<tr>').addClass('table-active').append(
        $('<th>').attr('colspan', '6').css({ 'padding-top': '10px', 'padding-bottom': '10px', 'text-align': 'unset' }).append(
          $('<span>').addClass('product_item_no').text(nextTableNumber + '. 新增品項')
        ),
        $('<td>').append(
          $('<div>').addClass('text-right').append(
            $('<a>').addClass('btn btn-danger btn-sm delete_button d-none').attr('style', 'color: white;').append(
              ...
              tables.each(function (index, table) {
                var number = index + 1;
                $(table).find('.product_item_no').text(number + '. 新增品項');
              });
              if (tables.length <= 1) {
      ...
      // 单行：寬度、高度、才數、數量、配件數量（只用placeholder，不用label）
      ...
      var newInput3 = $('<input>').attr({ type: 'text', readonly: '1', id: 'total_units', name: 'total_units', class: 'form-control formula_calculation val_required size_class', placeholder: '才數' }).css('width', '100%').on('input', function (e) {
      ...
      var newDiv4 = $('<div>').addClass('invalid-feedback').text('必填');
      newTd4.append(newInput4).append(newDiv4);
      // 配件數量（动态显示/隐藏）
      var newTd5 = $('<td class="accessory-input-td" style="width:25%;" data-name="accessory_qty"></td>');
      var accessoryInput = $('<input>').attr({ type: 'text', class: 'form-control formula_calculation quantity_class', id: 'accessory_qty', name: 'accessory_qty', placeholder: '配件數量', disabled: true }).css('width', '100%');
        ...
        $('<td colspan="3">').css('width', '50%').append(
          $('<div>').addClass('form-group').append(
            $('<select>').addClass('form-control product_category')
              .append(
                $('<option>').attr({ disabled: '1', selected: '1' }).text('產品類型'),  // 删掉「請選擇」
      ...
      // 新顺序如下：
      bannerTable.append(headerRow, uploadRow, fileNameRow, commentRow, paramInputRow, productSelectRow);
    ...
    showCustomerDetails: function (selectedCustomer) {
      ...
      var deliverySelect = $('<select>').addClass('form-control val_required').attr({ name: 'delivery_char', id: 'delivery_char' });
      ['自取', '快遞', '貨運', '施工'].forEach(function (opt) {
        deliverySelect.append($('<option>').attr('value', opt).text(opt).prop('selected', opt === '自取'));
      });
      ...
      $('input[name="search"]').after(horizontalLine, projectTitle, projectInput, deliveryRow, commentRow, projectTitle2);
