...
class Quotation(models.Model):
    ...
    product_domain = fields.Many2many('product.template', compute="_compute_product_domain")

    @api.depends('customer_class_id','product_id')
    def _compute_product_domain(self):
        for record in self:
            if record.customer_class_id:
                # 查找当前 customer_class_id 对应的所有 Quotation 中的 product_id
                related_products = self.search([('customer_class_id', '=', record.customer_class_id.id)]).mapped('product_id')
                # 将这些 product_id 写入 product_domain
                
                filtered_products = self.env['product.template'].search([
                    ('id', 'in', related_products.ids),
                    ('sale_ok', '=', True)
                ])
                record.product_domain = [(6, 0, filtered_products.ids)]
            else:
                # 如果没有 customer_class_id，则清空 domain
                record.product_domain = [(6, 0, [])]
